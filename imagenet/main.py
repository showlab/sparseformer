...
def main(config):
    ...
    try:
        input_img = torch.randn(1, 3, config.DATA.IMG_SIZE, config.DATA.IMG_SIZE).cuda()
        from fvcore.nn import FlopCountAnalysis, flop_count_table
        flops = FlopCountAnalysis(model_without_ddp, input_img)
        logger.info(flop_count_table(flops, max_depth=2))
    except ImportError:
        logger.info("Please install fvcore to obtain the model FLOPs")
        pass
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        if config.EMA.ENABLE:
            ...
            logger.info(f'[EMA] Max accuracy: {max_accuracy:.2f}%')

        model_to_valid = model
        acc1, acc5, loss = validate(
            config, data_loader_val, model_to_valid, epoch=epoch)
        logger.info(
            f"Accuracy of the network on the {len(dataset_val)} test images: {acc1:.1f}%")
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, model_ema=None):
    ...
    msg = f"EPOCH {epoch} training takes {datetime.timedelta(seconds=int(epoch_time))}, average loss: {loss_meter.avg:.4f})"
    logger.info(msg)
    # config.last_epoch_loss = loss_meter.avg
...
def validate(config, data_loader, model, epoch=0):
    ...
    msg = f'Epoch {epoch} * Acc@1 {acc1_meter.avg:.3f} Acc@5 {acc5_meter.avg:.3f}'
    logger.info(msg)
